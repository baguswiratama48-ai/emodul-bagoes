
import { Module } from './moduleContent';

export const kerajinanLimbahModule: Module = {
  id: 'kerajinan-limbah',
  title: 'Kerajinan dari Limbah',
  subtitle: 'Berbentuk Bangun Datar',
  description: 'Mempelajari konsep kerajinan dari bahan limbah berbentuk bangun datar, mengidentifikasi sumber ide kreatif, dan menentukan peluang usaha yang menjanjikan melalui pendekatan Project Based Learning.',
  objectives: [
    'Menjelaskan pengertian, jenis, dan karakteristik kerajinan dari bahan limbah berbentuk bangun datar',
    'Mengidentifikasi dan menganalisis berbagai sumber ide kreatif untuk produk kerajinan dari limbah bangun datar',
    'Menentukan dan memproyeksikan peluang usaha yang paling menjanjikan dari ide kerajinan limbah di pasar lokal atau digital',
  ],
  prerequisites: [
    'Mengidentifikasi berbagai jenis limbah yang terdapat di lingkungan sekitar',
    'Membedakan karakteristik bahan limbah berbentuk bangun datar (kertas, kardus, plastik lembaran, kain perca)',
    'Memahami konsep dasar kewirausahaan dan kreativitas',
  ],
  duration: '2 JP (2 x 45 menit)',
  sections: [
    {
      id: 'pengertian-limbah',
      title: 'Pengertian Limbah Bangun Datar',
      icon: '♻️',
      content: `
# Pengertian Limbah Bangun Datar

**Limbah Bangun Datar** adalah bahan sisa atau sampah yang memiliki bentuk dasar dua dimensi, seperti kertas, kardus, plastik lembaran, dan kain perca.

## Jenis-Jenis Limbah Bangun Datar:

### 1. Kertas dan Kardus 📦
- Koran bekas
- Majalah lama
- Kardus kemasan
- Kertas HVS bekas
- Karton bekas

### 2. Plastik Lembaran 🛍️
- Bungkus plastik sachet (kopi, detergen, dll)
- Tas plastik bekas
- Kantong kemasan
- Plastik pembungkus

### 3. Kain Perca 🧵
- Sisa potongan kain dari industri garmen
- Pakaian bekas yang tidak terpakai
- Sisa kain jahit
- Lap bekas

### 4. Limbah Lainnya 📋
- Daun kering
- Kulit jagung
- Serat alam

---

## Karakteristik Limbah Bangun Datar:

| Jenis Limbah | Karakteristik | Potensi Produk |
|--------------|---------------|----------------|
| Kertas/Kardus | Ringan, mudah dibentuk, bisa dilipat | Kotak penyimpanan, hiasan dinding, bingkai foto |
| Plastik Sachet | Anti air, kuat, warna beragam | Tas, dompet, tempat pensil |
| Kain Perca | Fleksibel, beragam tekstur | Tas, boneka, aksesori fashion |

---

## 💡 Pemahaman Bermakna

> "Limbah bukanlah akhir dari sebuah materi, melainkan awal dari peluang ekonomi yang berkelanjutan. Kreativitas dan analisis pasar yang tajam memungkinkan kita mengubah sampah yang tidak bernilai menjadi sumber penghasilan yang berharga (green economy) melalui inovasi kerajinan."
      `,
    },
    {
      id: 'upcycling',
      title: 'Konsep Upcycling',
      icon: '🔄',
      content: `
# Konsep Upcycling

## Apa itu Upcycling?

**Upcycling** adalah proses mengubah limbah atau produk yang tidak berguna menjadi bahan atau produk baru yang memiliki kualitas, kegunaan, atau nilai lingkungan yang **LEBIH BAIK** dari bentuk aslinya.

## Perbedaan Upcycling vs Recycling

| Aspek | Upcycling ♻️ | Recycling ♻️ |
|-------|-------------|-------------|
| Proses | Mengubah menjadi produk bernilai lebih tinggi | Mengolah kembali menjadi bahan dasar |
| Energi | Membutuhkan sedikit energi | Membutuhkan energi besar |
| Hasil | Produk unik dan kreatif | Bahan baku untuk produksi ulang |
| Contoh | Botol plastik → lampu hias | Botol plastik → biji plastik |

---

## Mengapa Upcycling Penting?

### 1. Mengurangi Limbah 🌍
Mengurangi jumlah sampah yang berakhir di tempat pembuangan akhir (TPA).

### 2. Hemat Sumber Daya 💚
Mengurangi kebutuhan bahan baku baru untuk produksi.

### 3. Peluang Ekonomi 💰
Menciptakan produk bernilai jual tinggi dari bahan yang gratis atau murah.

### 4. Ramah Lingkungan 🌱
Mengurangi emisi karbon dan jejak lingkungan.

---

## Green Economy (Ekonomi Hijau)

**Green Economy** adalah model ekonomi yang berfokus pada pembangunan berkelanjutan tanpa merusak lingkungan.

Wirausaha kerajinan dari bahan limbah (upcycling) merupakan salah satu bentuk wirausaha yang paling **ramah lingkungan dan relevan di masa depan** karena:

1. ✅ Mengurangi polusi limbah
2. ✅ Menciptakan lapangan kerja
3. ✅ Mendorong kreativitas dan inovasi
4. ✅ Mendukung ekonomi sirkular
5. ✅ Meningkatkan kesadaran lingkungan

---

## Contoh Sukses Upcycling di Indonesia

- **Tas dari bungkus kopi sachet** - Dijual hingga ratusan ribu rupiah
- **Furniture dari pallet bekas** - Diminati kalangan urban
- **Aksesori dari kain perca** - Populer di marketplace online
      `,
    },
    {
      id: 'ide-kreatif',
      title: 'Sumber Ide Kreatif',
      icon: '💡',
      content: `
# Sumber Ide Kreatif untuk Kerajinan Limbah

## Dari Mana Ide Kreatif Berasal?

### 1. Observasi Lingkungan 👀
Perhatikan limbah apa yang paling banyak di sekitar Anda:
- Rumah tangga
- Sekolah
- Kantor
- Pasar

### 2. Tren Pasar 📈
Amati produk kerajinan yang sedang populer:
- Marketplace (Tokopedia, Shopee)
- Media sosial (Instagram, TikTok)
- Pameran kerajinan

### 3. Kebutuhan Konsumen 🎯
Identifikasi masalah yang bisa dipecahkan:
- Kebutuhan penyimpanan
- Dekorasi rumah
- Fashion dan aksesori
- Hadiah unik

### 4. Riset dan Referensi 🔍
- Pinterest untuk inspirasi desain
- YouTube tutorial
- Artikel dan blog kreatif
- Kompetisi dan lomba kreativitas

---

## 10 Ide Kerajinan dari Limbah Bangun Datar

| No | Limbah | Ide Produk | Target Pasar |
|----|--------|------------|--------------|
| 1 | Plastik sachet kopi | Tas laptop | Mahasiswa, pekerja kantoran |
| 2 | Kardus bekas | Organizer meja | Pelajar, pekerja WFH |
| 3 | Kain perca | Masker fashion | Anak muda, dewasa |
| 4 | Koran bekas | Keranjang anyaman | Ibu rumah tangga |
| 5 | Plastik kemasan | Dompet multifungsi | Remaja, dewasa muda |
| 6 | Sisa banner | Tas belanja | Konsumen eco-friendly |
| 7 | Majalah bekas | Hiasan dinding | Pecinta dekorasi |
| 8 | Karton tebal | Bingkai foto | Semua kalangan |
| 9 | Kain flanel sisa | Gantungan kunci | Anak-anak, remaja |
| 10 | Plastik bening | Bunga artificial | Pengantin, event organizer |

---

## Proses Kreatif 5W + 1H

Gunakan pertanyaan ini untuk mengembangkan ide:

1. **WHAT** (Apa) - Produk apa yang akan dibuat?
2. **WHO** (Siapa) - Siapa target pembelinya?
3. **WHERE** (Di mana) - Di mana akan dijual?
4. **WHEN** (Kapan) - Kapan timing terbaik menjual?
5. **WHY** (Mengapa) - Mengapa konsumen harus membeli?
6. **HOW** (Bagaimana) - Bagaimana cara membuatnya?
      `,
    },
    {
      id: 'analisis-peluang',
      title: 'Analisis Peluang Usaha',
      icon: '📊',
      content: `
# Analisis Peluang Usaha Kerajinan Limbah

## Apa itu Peluang Usaha?

**Peluang Usaha** adalah kesempatan untuk memulai atau mengembangkan bisnis yang memiliki potensi untuk menghasilkan keuntungan, didasarkan pada kebutuhan pasar.

## Langkah Analisis Peluang Usaha

### 1. Identifikasi Produk ✅
- Tentukan jenis kerajinan yang akan dibuat
- Pilih limbah yang mudah didapat
- Pastikan memiliki nilai estetika dan fungsi

### 2. Riset Target Pasar 🎯
- Siapa yang akan membeli?
- Berapa mereka mau membayar?
- Di mana mereka biasa belanja?

### 3. Analisis Pesaing 🔍
- Siapa yang sudah menjual produk serupa?
- Apa kelebihan dan kekurangan mereka?
- Bagaimana cara unggul dari pesaing?

### 4. Tentukan USP (Unique Selling Point) ⭐
- Apa yang membedakan produk Anda?
- Mengapa konsumen harus memilih produk Anda?

---

## Analisis SWOT untuk Kerajinan Limbah

### Strengths (Kekuatan) 💪
- Bahan baku murah/gratis
- Produk unik dan berbeda
- Ramah lingkungan
- Margin keuntungan tinggi

### Weaknesses (Kelemahan) ⚠️
- Butuh keterampilan khusus
- Waktu produksi lama
- Kualitas tergantung ketersediaan limbah
- Persepsi "barang bekas"

### Opportunities (Peluang) 🌟
- Tren eco-friendly meningkat
- Pasar online berkembang
- Dukungan pemerintah untuk UMKM hijau
- Kesadaran lingkungan tinggi

### Threats (Ancaman) ⚡
- Persaingan meningkat
- Perubahan tren fashion
- Impor produk murah
- Konsistensi bahan baku

---

## Estimasi Peluang Pasar

| Tingkat | Kriteria | Contoh Produk |
|---------|----------|---------------|
| **Tinggi** | Permintaan tinggi, persaingan rendah | Tas laptop dari sachet |
| **Sedang** | Permintaan stabil, persaingan menengah | Dompet dari plastik |
| **Rendah** | Permintaan rendah, persaingan tinggi | Hiasan biasa |

---

## Tips Sukses Berbisnis Kerajinan Limbah

1. 🎨 **Fokus pada kualitas** - Jangan asal-asalan
2. 📸 **Foto produk menarik** - Penting untuk jualan online
3. 📱 **Manfaatkan media sosial** - Gratis dan efektif
4. 💬 **Ceritakan kisah produk** - Storytelling menjual
5. 🔄 **Terus berinovasi** - Jangan stagnan
      `,
    },
    {
      id: 'proses-produksi',
      title: 'Proses Produksi',
      icon: '🛠️',
      content: `
# Proses Produksi Kerajinan Limbah Bangun Datar

## Tahapan Produksi

### 1. Pengumpulan Bahan 📦
- Kumpulkan limbah dari berbagai sumber
- Pilih limbah yang masih layak pakai
- Bersihkan dari kotoran

### 2. Persiapan Bahan 🧹
- Cuci dan keringkan
- Potong sesuai ukuran
- Kelompokkan berdasarkan warna/jenis

### 3. Desain Produk ✏️
- Buat sketsa/rancangan
- Tentukan ukuran dan warna
- Siapkan pola/template

### 4. Proses Pengerjaan 🔧
- Potong bahan sesuai pola
- Rangkai/jahit/lem sesuai desain
- Berikan finishing

### 5. Quality Control ✅
- Periksa kerapian
- Pastikan kekuatan produk
- Cek estetika keseluruhan

### 6. Packaging 📦
- Kemas dengan menarik
- Tambahkan label/tag
- Siap dijual!

---

## Alat dan Bahan yang Dibutuhkan

### Alat Dasar:
- Gunting / Cutter
- Penggaris
- Lem (lem tembak, lem kayu, lem kain)
- Mesin jahit (untuk kain)
- Setrika
- Jarum dan benang

### Bahan Pendukung:
- Ritsleting
- Kancing
- Tali/webbing
- Karton pengeras
- Aksesoris tambahan

---

## Tips Produksi Berkualitas

| Aspek | Tips |
|-------|------|
| **Kebersihan** | Cuci bersih limbah sebelum diproses |
| **Kerapian** | Ukur dengan teliti, potong presisi |
| **Kekuatan** | Jahit/lem dengan kuat |
| **Estetika** | Perhatikan kombinasi warna dan pola |
| **Konsistensi** | Buat SOP (Standard Operating Procedure) |

---

## Contoh Proyek: Tas dari Plastik Sachet

### Bahan:
- 100+ bungkus sachet kopi bersih
- Ritsleting 30 cm
- Tali webbing 1 meter
- Kain furing

### Langkah:
1. Cuci dan keringkan sachet
2. Lipat masing-masing sachet menjadi strip
3. Anyam strip membentuk lembaran
4. Potong lembaran sesuai pola tas
5. Jahit bagian-bagian tas
6. Pasang ritsleting dan tali
7. Finishing dan quality control

**Estimasi waktu:** 4-6 jam kerja

**Harga jual:** Rp 75.000 - Rp 150.000
      `,
    },
    {
      id: 'pemasaran',
      title: 'Strategi Pemasaran',
      icon: '📣',
      content: `
# Strategi Pemasaran Kerajinan Limbah

## Platform Penjualan

### 1. Online Marketplace 🛒
- **Tokopedia** - Jangkauan luas
- **Shopee** - Fitur live shopping
- **Bukalapak** - UMKM friendly
- **Etsy** - Pasar internasional

### 2. Media Sosial 📱
- **Instagram** - Visual marketing
- **TikTok** - Video viral
- **Facebook** - Komunitas/grup
- **WhatsApp Business** - Komunikasi langsung

### 3. Offline 🏪
- Bazaar dan pameran
- Konsinyasi di toko
- Pasar kriya
- Event sekolah/kampus

---

## Teknik Marketing yang Efektif

### 1. Storytelling 📖
Ceritakan proses pembuatan dan dampak positif:
> "Setiap tas ini menyelamatkan 100+ bungkus plastik dari pencemaran"

### 2. Visual Menarik 📸
- Foto berkualitas tinggi
- Video proses pembuatan
- Before-after transformation
- User generated content

### 3. Eco-Branding 🌿
- Tekankan nilai ramah lingkungan
- Gunakan hashtag: #upcycling #gogreen #zerowaste
- Sertifikasi eco-friendly jika ada

### 4. Kolaborasi 🤝
- Influencer eco-friendly
- Brand sustainable
- Komunitas lingkungan
- Event CSR perusahaan

---

## Pricing Strategy (Strategi Harga)

### Metode Perhitungan:

**Harga Jual = Biaya Produksi + Margin Keuntungan**

| Komponen | Contoh |
|----------|--------|
| Bahan baku | Rp 5.000 |
| Tenaga kerja | Rp 30.000 |
| Overhead | Rp 5.000 |
| **Total Biaya** | **Rp 40.000** |
| Margin 100% | Rp 40.000 |
| **Harga Jual** | **Rp 80.000** |

### Tips Pricing:
- Riset harga kompetitor
- Jangan terlalu murah (kesan murahan)
- Sesuaikan dengan target pasar
- Berikan value added

---

## Membangun Brand

### Elemen Brand:
1. **Nama brand** yang mudah diingat
2. **Logo** yang menarik
3. **Tagline** yang memorable
4. **Packaging** yang instagram-worthy
5. **Konsistensi visual** di semua platform

### Contoh Nama Brand:
- EcoSac (dari sachet)
- Limbah Kreatif
- Green Craft ID
- Sampah Jadi Berkah
      `,
    },
    {
      id: 'tugas-catatan',
      title: 'Tugas Catatan',
      icon: '📝',
      subsections: [
        {
          id: 'pengertian-kerajinan',
          title: '1. Pengertian Kerajinan Limbah',
          content: `
# 1. Pengertian Kerajinan Limbah

**Kerajinan** adalah hal yang berkaitan dengan buatan tangan atau kegiatan yang berkaitan dengan barang yang dihasilkan melalui keterampilan tangan (kerajinan tangan).

**Limbah berbentuk bangun datar** adalah limbah yang berbentuk bangun yang berdimensi dua, yaitu bahan limbah yang memiliki sisi panjang dan lebar sehingga tidak mempunyai ruang. Limbah berbentuk bangun datar dapat berupa bidang beraturan seperti lingkaran, segi empat, segitiga, dan bangun tidak beraturan.

**Contoh limbah berbentuk bangun datar antara lain:**
• Daun
• Kertas
• Kain perca
• Plastik
          `,
        },
        {
          id: 'manfaat-limbah',
          title: '2. Pemanfaatan Limbah Bangun Datar',
          content: `
# 2. Kerajinan dengan Memanfaatkan Bahan dari Limbah Berbentuk Bangun Datar

### 1. Kerajinan dari Limbah Kulit Jagung
Kulit jagung merupakan limbah pertanian dari tanaman jagung. Kulit jagung kerap kali tidak diperhatikan, bahkan dianggap sampah sehingga biasanya dibuang. Sampah kulit jagung bisa menjadi benda kerajinan yang sangat bernilai dan bisa mendatangkan keuntungan. Kerajinan dari limbah kulit jagung ini sangat unik dan menarik.

### 2. Kerajinan dari Limbah Daun Pelepah Pisang
Sebagian besar masyarakat menganggap daun pelepah pisang kering adalah sampah yang tidak berguna. Bahkan terkadang daun pelepah pisang kering hanya dibakar begitu saja karena dianggap sampah yang mengotori kebun. Namun, daun pelepah pisang kering dapat dijadikan sebagai kerajinan yang indah dan memiliki nilai ekonomi yang tinggi. Kerajinan tangan dari daun pelepah pisang kering sebaiknya menggunakan daun yang berwarna kuning hingga daun yang berwarna cokelat dan benar-benar kering.

### 3. Kerajinan dari Limbah Plastik
Apakah di lingkunganmu banyak sampah plastik yang menumpuk? Kalian bingung untuk memanfaatkannya? Kita tahu bahwa sampah plastik termasuk dalam sampah anorganik yang sulit diurai oleh mikroorganisme. Butuh waktu bertahun-tahun supaya plastik dapat terurai dan akhirnya menyatu dengan tanah. Oleh karena itu, sebaiknya sampah plastik tersebut dimanfaatkan untuk karya kerajinan. Saat ini sudah banyak kerajinan yang dibuat dengan bahan dasar limbah plastik seperti tas, dompet, cover meja, dan tempat tisu.

### 4. Kerajinan dari Limbah Kertas
Kertas merupakan bagian dari limbah organik kering. Hal ini karena kertas mudah terurai dalam tanah. Meskipun kertas mudah hancur jika terkena air, namun jika digunakan sebagai bahan dasar produk kerajinan, kertas tersebut dapat diolah sedemikian rupa sehingga tidak mudah hancur. Caranya yaitu dengan menambah kandungan lem atau zat pelindung anti air seperti melamin/politur, dapat pula dengan dilapisi plastik. Limbah kertas dapat digunakan sebagai benda kerajinan dengan berbagai teknik seperti teknik anyaman, teknik sobek, teknik lipat, dan teknik gulung (pilin).

### 5. Kerajinan dari Limbah Kain Perca
Kebutuhan sandang manusia yang berupa pakaian merupakan kebutuhan primer sehari-hari yang harus dipenuhi. Produksi pakaian yang dilakukan oleh para penjahit atau konveksi menghasilkan banyak limbah kain yang biasa disebut kain perca. Limbah kain perca dapat dibuat sebagai bahan dasar kerajinan yang cukup unik dan menarik. Bahkan busana itu sendiri dapat dihasilkan dari kain-kain perca yang dijahit bersambung-sambungan. Bagi sebagian orang, ada juga yang berminat pada busana jenis ini karena unik. Sekarang sudah semakin banyak orang melirik produk kerajinan berbahan kain perca karena selain murah juga desainnya unik.

### 6. Kerajinan dari Limbah Kardus
Kardus bekas terkadang menjadi sebuah benda yang menumpuk dan memenuhi ruangan sehingga mengganggu pemandangan, dan kadang dibuang begitu saja di tempat sampah. Saat ini banyak sekali kardus bekas yang ada di lingkungan kita. Hal ini dikarenakan hampir setiap barang kebutuhan kita sehari-hari menggunakan kardus sebagai pembungkusnya. Bagi seseorang yang berjiwa wirausaha, maka alangkah lebih bagus bila kardus tersebut dimanfaatkan menjadi sebuah kerajinan unik dan juga memiliki nilai seni yang tinggi.

### 7. Kerajinan dari Limbah Sisik Ikan
Sisik ikan pada umumnya hanya dibuang karena dianggap sebagai limbah yang tidak bermanfaat. Ternyata sisik ikan dapat dimanfaatkan untuk benda kerajinan, pada umumnya untuk kerajinan aksesori. Setiap ikan menghasilkan sisik yang berbeda ukuran dan ketebalannya. Sisik ikan kakap sering digunakan sebagai produk kerajinan karena sisiknya terlihat kokoh, tebal, dan besar dibanding sisik ikan mas atau mujair. Limbah sisik ikan bisa dijadikan sebagai bahan utama pembuatan aksesori seperti anting-anting, cincin, kalung, bros, dan gelang. Hasilnya terlihat unik, artistik, dan menarik.

### 8. Kerajinan dari Pecahan Keramik
Pecahan keramik ternyata dapat dimanfaatkan untuk kerajinan atau hiasan. Pecahan-pecahan keramik dapat dijadikan sebagai hiasan mozaik atau hiasan yang lainnya. Biasanya mozaik dari pecahan keramik disusun untuk membuat gambar bercorak abstrak atau background dari suatu gambar atau untuk melapisi dinding dan lantai agar terkesan unik.
          `,
        },
        {
          id: 'contoh-wilayah',
          title: '3. Contoh Berdasarkan Wilayah',
          content: `
# 3. Contoh Limbah Berbentuk Bangun Datar Berdasarkan Kondisi Wilayah

### 1. Daerah pesisir pantai atau laut
Limbah berbentuk bangun datar yang banyak tersedia adalah sisik ikan, daun pandan, daun kelapa, dan lainnya.

### 2. Daerah pegunungan
Limbah berbentuk bangun datar yang banyak dihasilkan di daerah ini adalah daun-daunan kering, kulit buah-buahan yang bertekstur keras seperti salak, kulit pete, dan lainnya.

### 3. Daerah pertanian
Limbah berbentuk bangun datar yang didapat pada daerah ini seperti jerami padi, kulit jagung, batang, daun singkong, kulit bawang, dan lainnya.

### 4. Daerah perkotaan
Limbah berbentuk bangun datar yang dihasilkan di daerah perkotaan biasanya kertas, kardus, serbuk gergaji, serutan kayu, plastik, mika, dan lainnya.

> Berbagai macam limbah berbentuk bangun datar sangat bermanfaat untuk bahan pembuatan produk kerajinan. Dengan demikian, kita sebagai manusia yang memiliki akal sehat seharusnya lebih peduli dengan lingkungan kita sendiri. Mulailah dengan kebiasaan tidak membuang sampah sembarangan dan teruslah berpikir untuk terus berinovasi dalam rangka pengurangan limbah dan memanfaatkannya menjadi barang yang bernilai ekonomi tinggi.
          `,
        },
        {
          id: 'ide-peluang',
          title: '4. Ide dan Peluang Usaha',
          content: `
# 4. Ide dan Peluang Usaha

Limbah terkumpul dan menumpuk di sekitar kita setiap hari, mulai dari yang sifatnya organik dan nonorganik. Salah satu penyumbang limbah terbesar adalah kertas yang sudah tak terpakai. Kertas koran menjadi salah satu limbah kertas rumah tangga yang mendominasi. Koran bekas yang sudah lewat dari tanggalnya sering ditelantarkan begitu saja oleh pemiliknya. Sebagian digunakan sebagai pembungkus makanan, pembungkus paket, dan beberapa hal lainnya yang nantinya akan berakhir di tempat sampah juga.

Pertanyaan akan muncul di benak kita, haruskah limbah kertas yang melimpah itu berakhir begitu saja di tempat sampah yang bahkan tak bisa menampungnya? Sudah kita ketahui bahwa penggunaan kertas berkala secara tidak langsung telah memengaruhi pula lingkungan sekitar kita.

Sebagai generasi muda, sudah sewajarnya kita peduli terhadap lingkungan. Lingkungan inilah yang akan menjadi penentu masa depan generasi selanjutnya. Wujud kepedulian yang bisa kita lakukan untuk lingkungan salah satunya dengan mendaur ulang limbah kertas menjadi lebih bermanfaat.

Berawal dari ide kreatif dan unik, proses daur ulang tidak hanya dapat menambah nilai manfaat dan keindahan barang yang dihasilkan, tetapi juga dapat meningkatkan nilai ekonominya dan membantu perekonomian masyarakat sekitar. Produk kreatif berawal dari ide yang kreatif pula.

**Beberapa hal yang harus diperhatikan dalam menciptakan produk kreatif:**
1. Ide dalam pembuatan produk kerajinan yang diminati konsumen
2. Ide dalam pembuatan produk kerajinan yang dapat memenangkan persaingan
3. Ide dalam pembuatan dan pendayagunaan sumber-sumber produk kerajinan
4. Ide yang dapat mencegah kebosanan konsumen dalam penggunaan produk kerajinan

**Peluang Usaha** itu sendiri dapat diartikan sebagai suatu ide yang menarik atau usulan usaha yang memberi kemungkinan untuk memberikan hasil bagi investor atau pihak yang mengambil risiko. Peluang usaha digambarkan oleh persyaratan dan mengarah ke penyediaan suatu usaha produk yang dibuat atau ditambahkan nilainya untuk keperluan pembeli.

**Suatu peluang usaha yang baik mampu memenuhi beberapa kriteria sebagai berikut:**
1. Permintaan yang nyata
2. Pengembalian investasi
3. Kompetitif
4. Mencapai tujuan
5. Ketersediaan sumber daya dan keterampilan

> Berdasarkan materi di atas, maka peluang yang dapat diambil adalah pemanfaatan limbah tidak terpakai berbentuk bangun datar yang keberadaannya melimpah di sekitar kita. Sebagai bentuk usaha, bisnis kerajinan limbah bahan kertas cukup menjanjikan karena bahan dasar banyak tersedia dan mudah diperoleh. Dengan mengambil ide dari benda-benda hias dan fungsional, limbah kertas dapat dibuat sebagai kerajinan. Dengan begitu, kita membantu mengurangi jumlah limbah kertas dan mendapatkan peluang usaha dengan memanfaatkan sumber daya material yang ada di sekitar lingkungan kita.
          `,
        },
        {
          id: 'tugas-analisis',
          title: '5. Tugas Analisis Mandiri (OKU Timur)',
          content: `
# 5. Tugas Analisis Mandiri: Potensi OKU Timur

Silakan tulis dan jawab 5 pertanyaan berikut di buku catatan Anda untuk menganalisis potensi kerajinan di daerah kita!

### Soal 1: Identifikasi Limbah Lokal
Kabupaten OKU Timur dikenal sebagai daerah pertanian dan perkebunan yang subur. Coba amati lingkungan sekitarmu, sebutkan **3 jenis limbah bangun datar** yang paling mudah ditemukan di OKU Timur dan jelaskan dari mana limbah tersebut berasal! (Contoh: Kulit jagung dari sisa panen).

### Soal 2: Analisis Peluang Usaha
Dari limbah-limbah yang sudah kamu sebutkan di nomor 1, pilih salah satu yang menurutmu paling berpotensi untuk dijadikan bisnis kerajinan di OKU Timur. Jelaskan **alasanmu memilih limbah tersebut** (misalnya: bahannya gratis, mudah dibentuk, atau belum banyak pesaing).

### Soal 3: Ide Produk Inovatif
Jika kamu harus membuat satu produk kerajinan tangan khas OKU Timur yang unik untuk dipamerkan di festival daerah, **produk apa yang akan kamu buat**? Gambarkan sedikit tentang bentuk dan fungsinya!

### Soal 4: Strategi Pemasaran Lokal
Bagaimana caramu mempromosikan produk kerajinanmu agar dikenal oleh masyarakat OKU Timur dan sekitarnya? Sebutkan strategi yang menurutmu paling efektif (misalnya lewat media sosial, titip di toko oleh-oleh, atau bazar).

### Soal 5: Mengatasi Tantangan
Menurut pendapatmu, apa **tantangan atau kesulitan terbesar** dalam memulai usaha kerajinan di daerah kita? Dan bagaimana solusi cerdas yang bisa kamu tawarkan untuk mengatasinya?

> "Kesuksesan dimulai dari kepekaan melihat peluang di sekitar kita. Jadilah anak muda OKU Timur yang kreatif dan solutif!"
          `,
        },
      ],
    },
  ],
  videos: [
    {
      id: 'video-pkwu-1',
      title: 'Kerajinan dari Bahan Limbah Berbentuk Bangun Datar',
      url: 'https://www.youtube.com/embed/YXZNNnRKk80',
      description: 'Video pembelajaran tentang kerajinan dari bahan limbah berbentuk bangun datar untuk mata pelajaran PKWU kelas XI',
    },
  ],
  quizQuestions: [
    {
      id: 'pkwu-q1',
      question: 'Apa yang dimaksud dengan limbah bangun datar?',
      options: [
        'Limbah yang berbentuk tiga dimensi',
        'Bahan sisa yang memiliki bentuk dasar dua dimensi seperti kertas, kardus, dan plastik lembaran',
        'Limbah yang berasal dari industri konstruksi',
        'Sampah organik yang dapat terurai',
      ],
      correctAnswer: 1,
      explanation: 'Limbah bangun datar adalah bahan sisa atau sampah yang memiliki bentuk dasar dua dimensi, seperti kertas, kardus, plastik lembaran, dan kain perca.',
    },
    {
      id: 'pkwu-q2',
      question: 'Apa perbedaan utama antara upcycling dan recycling?',
      options: [
        'Upcycling membutuhkan lebih banyak energi',
        'Upcycling mengubah limbah menjadi produk bernilai lebih tinggi',
        'Recycling menghasilkan produk yang lebih berkualitas',
        'Tidak ada perbedaan, keduanya sama',
      ],
      correctAnswer: 1,
      explanation: 'Upcycling mengubah limbah menjadi produk bernilai lebih tinggi dari bentuk aslinya, sementara recycling mengolah kembali menjadi bahan dasar.',
    },
    {
      id: 'pkwu-q3',
      question: 'Mengapa wirausaha kerajinan dari bahan limbah disebut ramah lingkungan?',
      options: [
        'Karena menggunakan bahan impor',
        'Karena mengurangi limbah dan mendukung ekonomi sirkular',
        'Karena harganya mahal',
        'Karena hanya dijual offline',
      ],
      correctAnswer: 1,
      explanation: 'Wirausaha kerajinan limbah ramah lingkungan karena mengurangi limbah, menghemat sumber daya, dan mendukung konsep ekonomi hijau (green economy).',
    },
    {
      id: 'pkwu-q4',
      question: 'Apa yang dimaksud dengan Green Economy?',
      options: [
        'Ekonomi yang fokus pada perdagangan sayuran',
        'Model ekonomi yang berfokus pada pembangunan berkelanjutan tanpa merusak lingkungan',
        'Sistem ekonomi berbasis pertanian',
        'Ekonomi yang hanya menggunakan warna hijau dalam branding',
      ],
      correctAnswer: 1,
      explanation: 'Green Economy adalah model ekonomi yang berfokus pada pembangunan berkelanjutan tanpa merusak lingkungan, di mana bisnis limbah termasuk di dalamnya.',
    },
    {
      id: 'pkwu-q5',
      question: 'Apa yang dimaksud dengan USP (Unique Selling Point)?',
      options: [
        'Harga jual produk',
        'Keunikan yang membedakan produk dari pesaing',
        'Jumlah produk yang dijual',
        'Lokasi penjualan',
      ],
      correctAnswer: 1,
      explanation: 'USP (Unique Selling Point) adalah keunikan atau nilai tambah yang membedakan produk Anda dari produk pesaing, menjadi alasan mengapa konsumen harus memilih produk Anda.',
    },
    {
      id: 'pkwu-q6',
      question: 'Limbah plastik sachet paling cocok dibuat menjadi produk apa?',
      options: [
        'Pupuk organik',
        'Tas atau dompet karena sifatnya anti air dan kuat',
        'Makanan olahan',
        'Obat-obatan',
      ],
      correctAnswer: 1,
      explanation: 'Plastik sachet cocok untuk tas atau dompet karena karakteristiknya yang anti air, kuat, ringan, dan memiliki warna/merk yang bervariasi.',
    },
    {
      id: 'pkwu-q7',
      question: 'Dalam analisis SWOT, tren eco-friendly yang meningkat termasuk kategori...',
      options: [
        'Strength (Kekuatan)',
        'Weakness (Kelemahan)',
        'Opportunity (Peluang)',
        'Threat (Ancaman)',
      ],
      correctAnswer: 2,
      explanation: 'Tren eco-friendly yang meningkat adalah faktor eksternal yang menguntungkan, sehingga termasuk kategori Opportunity (Peluang).',
    },
    {
      id: 'pkwu-q8',
      question: 'Apa keuntungan utama berjualan di marketplace online seperti Tokopedia atau Shopee?',
      options: [
        'Tidak perlu modal sama sekali',
        'Jangkauan pasar yang luas dan kemudahan akses pembeli',
        'Barang pasti habis terjual',
        'Tidak ada persaingan',
      ],
      correctAnswer: 1,
      explanation: 'Marketplace online memberikan keuntungan jangkauan pasar yang luas dan kemudahan akses bagi pembeli dari berbagai daerah.',
    },
    {
      id: 'pkwu-q9',
      question: 'Teknik storytelling dalam marketing bertujuan untuk...',
      options: [
        'Menurunkan harga produk',
        'Menceritakan proses pembuatan dan dampak positif untuk membangun koneksi emosional',
        'Menyembunyikan asal bahan',
        'Mengurangi biaya produksi',
      ],
      correctAnswer: 1,
      explanation: 'Storytelling bertujuan untuk menceritakan proses pembuatan dan dampak positif produk, sehingga membangun koneksi emosional dengan konsumen.',
    },
    {
      id: 'pkwu-q10',
      question: 'Jika biaya produksi sebuah tas dari limbah adalah Rp 40.000 dan margin keuntungan 100%, berapa harga jualnya?',
      options: [
        'Rp 40.000',
        'Rp 60.000',
        'Rp 80.000',
        'Rp 100.000',
      ],
      correctAnswer: 2,
      explanation: 'Harga jual = Biaya produksi + Margin. Dengan margin 100%, maka Rp 40.000 + (100% × Rp 40.000) = Rp 80.000.',
    },
    {
      id: 'pkwu-q11',
      question: 'Kerajinan adalah kegiatan yang berkaitan dengan barang yang dihasilkan melalui...',
      options: [
        'Mesin otomatis',
        'Keterampilan tangan',
        'Cetakan pabrik',
        'Perangkat lunak komputer',
      ],
      correctAnswer: 1,
      explanation: 'Kerajinan tangan (handicraft) adalah kegiatan yang berkaitan dengan barang yang dihasilkan melalui keterampilan tangan.',
    },
    {
      id: 'pkwu-q12',
      question: 'Limbah berbentuk bangun datar memiliki dimensi...',
      options: [
        'Satu dimensi (panjang)',
        'Dua dimensi (panjang dan lebar)',
        'Tiga dimensi (panjang, lebar, tinggi)',
        'Tidak berdimensi',
      ],
      correctAnswer: 1,
      explanation: 'Limbah bangun datar adalah limbah yang berdimensi dua, yaitu memiliki sisi panjang dan lebar sehingga tidak mempunyai ruang.',
    },
    {
      id: 'pkwu-q13',
      question: 'Manakah di bawah ini yang BUKAN merupakan contoh limbah bangun datar?',
      options: [
        'Kain perca',
        'Kertas bekas',
        'Botol plastik utuh',
        'Daun kering',
      ],
      correctAnswer: 2,
      explanation: 'Botol plastik utuh memiliki ruang/volume, sehingga termasuk limbah bangun ruang (tiga dimensi). Kain, kertas, dan daun adalah bangun datar.',
    },
    {
      id: 'pkwu-q14',
      question: 'Limbah kulit jagung seringkali dianggap sampah, padahal bisa dimanfaatkan untuk...',
      options: [
        'Pakan ternak saja',
        'Bahan bangunan',
        'Benda kerajinan bernilai seni',
        'Bahan bakar kendaraan',
      ],
      correctAnswer: 2,
      explanation: 'Kulit jagung yang biasanya dibuang dapat dimanfaatkan menjadi benda kerajinan yang unik dan bernilai ekonomi tinggi.',
    },
    {
      id: 'pkwu-q15',
      question: 'Warna daun pelepah pisang yang paling baik digunakan untuk kerajinan adalah...',
      options: [
        'Hijau segar',
        'Hijau muda',
        'Kuning hingga cokelat kering',
        'Hitam busuk',
      ],
      correctAnswer: 2,
      explanation: 'Untuk kerajinan, sebaiknya menggunakan daun pelepah pisang yang sudah berwarna kuning hingga cokelat dan benar-benar kering.',
    },
    {
      id: 'pkwu-q16',
      question: 'Mengapa limbah plastik harus dimanfaatkan menjadi kerajinan?',
      options: [
        'Karena plastik mudah terurai',
        'Karena plastik sulit diurai mikroorganisme',
        'Karena plastik harganya mahal',
        'Karena plastik adalah bahan organik',
      ],
      correctAnswer: 1,
      explanation: 'Sampah plastik adalah anorganik yang sulit diurai (butuh ratusan tahun). Memanfaatkannya menjadi kerajinan membantu mengurangi pencemaran.',
    },
    {
      id: 'pkwu-q17',
      question: 'Agar limbah kertas tidak mudah hancur saat dijadikan kerajinan, teknik apa yang bisa digunakan?',
      options: [
        'Direndam air panas',
        'Dibakar setengah matang',
        'Ditambah lem atau pelapis anti air',
        'Dibiarkan di tempat lembab',
      ],
      correctAnswer: 2,
      explanation: 'Limbah kertas bisa diperkuat dengan menambahkan kandungan lem, melamin, politur, atau dilapisi plastik agar tahan air dan tidak mudah hancur.',
    },
    {
      id: 'pkwu-q18',
      question: 'Kain perca adalah limbah yang berasal dari...',
      options: [
        'Sisa makanan',
        'Industri konveksi atau penjahit',
        'Perkebunan kapas',
        'Pabrik plastik',
      ],
      correctAnswer: 1,
      explanation: 'Kain perca adalah sisa-sisa potongan kain yang dihasilkan dari kegiatan menjahit atau industri konveksi.',
    },
    {
      id: 'pkwu-q19',
      question: 'Teknik pembuatan kerajinan dari kain perca sering disebut dengan teknik...',
      options: [
        'Patchwork (menjahit sambung)',
        'Ukiran',
        'Anyaman rotan',
        'Pahat batu',
      ],
      correctAnswer: 0,
      explanation: 'Kerajinan kain perca sering dibuat dengan teknik menjahit sambung-menyambung potongan kain kecil, atau dikenal sebagai patchwork.',
    },
    {
      id: 'pkwu-q20',
      question: 'Mengapa kardus bekas banyak tersedia di lingkungan kita?',
      options: [
        'Karena kardus adalah bahan bangunan utama',
        'Karena hampir setiap barang kebutuhan menggunakan kardus sebagai pembungkus',
        'Karena kardus tumbuh di pohon',
        'Karena kardus tidak bisa rusak',
      ],
      correctAnswer: 1,
      explanation: 'Kardus bekas melimpah karena hampir semua produk kemasan (elektronik, makanan, barang paket) menggunakan kardus sebagai pelindung.',
    },
    {
      id: 'pkwu-q21',
      question: 'Sisik ikan kakap lebih sering dipilih untuk kerajinan dibandingkan ikan mas karena...',
      options: [
        'Sisiknya lebih kecil dan tipis',
        'Sisiknya kokoh, tebal, dan besar',
        'Sisiknya berbau harum',
        'Sisiknya berwarna hitam pekat',
      ],
      correctAnswer: 1,
      explanation: 'Sisik ikan kakap memiliki karakteristik yang kokoh, tebal, dan ukurannya besar sehingga lebih mudah dibentuk menjadi aksesoris.',
    },
    {
      id: 'pkwu-q22',
      question: 'Contoh produk kerajinan yang bisa dibuat dari limbah sisik ikan adalah...',
      options: [
        'Meja dan kursi',
        'Aksesoris seperti anting dan bros',
        'Pakaian olahraga',
        'Alat masak',
      ],
      correctAnswer: 1,
      explanation: 'Limbah sisik ikan sangat cocok dijadikan aksesoris perhiasan wanita seperti anting, bros, kalung, dan gelang karena keindahannya.',
    },
    {
      id: 'pkwu-q23',
      question: 'Pecahan keramik dapat disusun membentuk gambar atau pola abstrak. Teknik ini disebut...',
      options: [
        'Mozaik',
        'Batik',
        'Tenun',
        'Relief',
      ],
      correctAnswer: 0,
      explanation: 'Seni menyusun pecahan material (seperti kaca atau keramik) menjadi sebuah gambar atau pola disebut seni mozaik.',
    },
    {
      id: 'pkwu-q24',
      question: 'Limbah bangun datar yang banyak ditemukan di daerah pesisir pantai adalah...',
      options: [
        'Kulit jagung dan jerami',
        'Sisik ikan, daun pandan, dan daun kelapa',
        'Kardus dan plastik kemasan',
        'Serbuk gergaji kayu',
      ],
      correctAnswer: 1,
      explanation: 'Di daerah pesisir pantai, limbah yang dominan adalah hasil laut (sisik ikan) dan tanaman pantai (kelapa, pandan).',
    },
    {
      id: 'pkwu-q25',
      question: 'Di daerah pegunungan, jenis limbah bangun datar yang potensial adalah...',
      options: [
        'Sisik ikan',
        'Plastik pabrik',
        'Daun-daunan kering dan kulit buah bertekstur keras',
        'Kertas perkantoran',
      ],
      correctAnswer: 2,
      explanation: 'Daerah pegunungan yang banyak pepohonan menghasilkan limbah daun kering dan kulit buah-buahan keras (seperti salak, pete).',
    },
    {
      id: 'pkwu-q26',
      question: 'Limbah jerami padi, batang singkong, dan kulit bawang banyak ditemukan di wilayah...',
      options: [
        'Perkotaan',
        'Pesisir',
        'Pertanian',
        'Pegunungan kapur',
      ],
      correctAnswer: 2,
      explanation: 'Jerami, kulit bawang, dan batang singkong adalah sisa-sisa hasil panen yang khas ditemukan di daerah pertanian.',
    },
    {
      id: 'pkwu-q27',
      question: 'Di daerah perkotaan, limbah bangun datar yang mendominasi berasal dari...',
      options: [
        'Sisa panen padi',
        'Kegiatan industri, perkantoran, dan rumah tangga (kertas, kardus, plastik)',
        'Tangkapan nelayan',
        'Guguran daun hutan',
      ],
      correctAnswer: 1,
      explanation: 'Perkotaan sebagai pusat aktivitas ekonomi menghasilkan banyak limbah kertas, kardus, dan plastik dari perkantoran dan pertokoan.',
    },
    {
      id: 'pkwu-q28',
      question: 'Apa tujuan utama dari inovasi produk kerajinan limbah?',
      options: [
        'Menambah jumlah sampah',
        'Meningkatkan nilai ekonomi dan mengurangi limbah',
        'Membuat lingkungan makin kotor',
        'Menghabiskan uang modal',
      ],
      correctAnswer: 1,
      explanation: 'Inovasi bertujuan mengubah sampah tak bernilai menjadi barang bernilai ekonomi tinggi (cuan) sekaligus membersihkan lingkungan.',
    },
    {
      id: 'pkwu-q29',
      question: 'Salah satu kriteria peluang usaha yang baik adalah "Permintaan yang nyata". Artinya...',
      options: [
        'Produk hanya disukai pembuatnya',
        'Ada pembeli yang benar-benar membutuhkan dan mau membeli produk tersebut',
        'Produk dijual gratis',
        'Produk tidak laku di pasaran',
      ],
      correctAnswer: 1,
      explanation: 'Permintaan nyata berarti produk tersebut memang dicari dan dibutuhkan oleh pasar, bukan hanya sekadar ide tanpa pembeli.',
    },
    {
      id: 'pkwu-q30',
      question: 'Ide produk kreatif sebaiknya muncul dari...',
      options: [
        'Meniru persis karya orang lain',
        'Keinginan untuk memenangkan persaingan dan mencegah kebosanan konsumen',
        'Paksaan orang tua',
        'Mimpi kosong',
      ],
      correctAnswer: 1,
      explanation: 'Produk kreatif lahir dari ide untuk menawarkan sesuatu yang baru, berbeda, dan solutif agar konsumen tidak bosan dan produk bisa bersaing.',
    },
    {
      id: 'pkwu-q31',
      question: 'Limbah kertas koran di rumah tangga seringkali berakhir sebagai...',
      options: [
        'Pakan ikan',
        'Pembungkus makanan atau dibuang begitu saja',
        'Bahan bangunan utama',
        'Pupuk cair',
      ],
      correctAnswer: 1,
      explanation: 'Koran bekas seringkali hanya dipakai untuk membungkus nasi/paket atau ditumpuk sampai berdebu lalu dibuang.',
    },
    {
      id: 'pkwu-q32',
      question: 'Satu langkah awal kepedulian generasi muda terhadap lingkungan adalah...',
      options: [
        'Membakar sampah di tempat terbuka',
        'Membuang sampah ke sungai',
        'Mendaur ulang limbah kertas menjadi barang bermanfaat',
        'Acuh tak acuh terhadap kebersihan',
      ],
      correctAnswer: 2,
      explanation: 'Mendaur ulang (recycle/upcycle) adalah tindakan nyata kepedulian lingkungan yang bisa dilakukan pelajar.',
    },
    {
      id: 'pkwu-q33',
      question: 'Peluang usaha dapat diartikan sebagai...',
      options: [
        'Risiko kerugian yang besar',
        'Kesempatan untuk memulai bisnis yang berpotensi laba',
        'Kegiatan menghabiskan waktu luang',
        'Hobi yang tidak menghasilkan uang',
      ],
      correctAnswer: 1,
      explanation: 'Peluang usaha adalah kesempatan (opportunity) untuk menjalankan kegiatan bisnis yang menghasilkan keuntungan/laba.',
    },
    {
      id: 'pkwu-q34',
      question: 'Apa yang dimaksud dengan "Nilai Tambah" dalam produk kerajinan?',
      options: [
        'Harga barang yang semakin murah',
        'Perubahan dari bahan mentah/sampah menjadi produk jadi yang lebih berguna dan mahal',
        'Menambahkan berat barang',
        'Menambah jumlah sampah',
      ],
      correctAnswer: 1,
      explanation: 'Nilai tambah (value added) terjadi ketika kita mengolah bahan baku (sampah) menjadi produk baru yang harganya jauh lebih tinggi.',
    },
    {
      id: 'pkwu-q35',
      question: 'Limbah mika dan serutan kayu biasanya banyak ditemukan di daerah...',
      options: [
        'Hutan lindung',
        'Perkotaan',
        'Laut dalam',
        'Sawah',
      ],
      correctAnswer: 1,
      explanation: 'Mika (plastik) dan serutan kayu (dari industri mebel/konstruksi) adalah limbah khas area perkotaan/industri.',
    },
    {
      id: 'pkwu-q36',
      question: 'Salah satu tantangan membuat kerajinan dari bahan alam (seperti daun) adalah...',
      options: [
        'Bahannya terlalu mahal',
        'Mudah rusak atau busuk jika pengolahan tidak benar',
        'Warnanya terlalu mencolok',
        'Sangat berat',
      ],
      correctAnswer: 1,
      explanation: 'Bahan alam seperti daun/kulit buah rentan terhadap jamur dan pembusukan, sehingga perlu teknik pengeringan dan pengawetan yang tepat.',
    },
    {
      id: 'pkwu-q37',
      question: 'Dalam analisis SWOT, kesulitan mendapatkan bahan baku yang konsisten termasuk dalam...',
      options: [
        'Strength',
        'Weakness',
        'Opportunity',
        'Threat',
      ],
      correctAnswer: 1,
      explanation: 'Kesulitan internal seperti suplai bahan baku yang tidak stabil adalah Kelemahan (Weakness) dari usaha tersebut.',
    },
    {
      id: 'pkwu-q38',
      question: 'Jika kamu tinggal di dekat pasar tradisional, limbah bangun datar apa yang paling mungkin kamu manfaatkan?',
      options: [
        'Kardus buah dan plastik kemasan',
        'Limbah nuklir',
        'Besi tua',
        'Pecahan satelit',
      ],
      correctAnswer: 0,
      explanation: 'Pasar tradisional menghasilkan banyak limbah kardus bekas wadah buah/barang dan plastik kemasan.',
    },
    {
      id: 'pkwu-q39',
      question: 'Sebuah produk kerajinan dikatakan "Fungsional" jika...',
      options: [
        'Hanya bisa dipajang',
        'Memiliki kegunaan praktis (bisa dipakai)',
        'Harganya sangat mahal',
        'Terbuat dari emas',
      ],
      correctAnswer: 1,
      explanation: 'Produk fungsional adalah produk yang memiliki fungsi pakai, seperti tas (untuk bawa barang), tempat tisu, atau dompet.',
    },
    {
      id: 'pkwu-q40',
      question: 'Untuk membuat kerajinan bunga dari kulit jagung, langkah pertama yang harus dilakukan pada kulit jagung adalah...',
      options: [
        'Langsung dilem',
        'Direbus dengan pewarna tekstil (opsional) dan dikeringkan',
        'Dibakar sampai hangus',
        'Dicincang halus',
      ],
      correctAnswer: 1,
      explanation: 'Kulit jagung perlu dibersihkan, bisa direbus dengan pewarna agar menarik, lalu dikeringkan (disetrika) agar lembaran rata dan mudah dibentuk.',
    },
    {
      id: 'pkwu-q41',
      question: 'Apa fungsi utama dari "Kemasan" (Packaging) produk kerajinan?',
      options: [
        'Agar produk tidak terlihat',
        'Melindungi produk dan menambah daya tarik jual',
        'Memberatkan produk',
        'Membuat produk cepat rusak',
      ],
      correctAnswer: 1,
      explanation: 'Kemasan berfungsi melindungi produk dari kerusakan saat dibawa dan mempercantik tampilan agar konsumen lebih tertarik membeli.',
    },
    {
      id: 'pkwu-q42',
      question: 'Strategi pemasaran "dari mulut ke mulut" masih efektif karena...',
      options: [
        'Tidak memerlukan biaya (gratis) dan terpercaya',
        'Membutuhkan internet kencang',
        'Hanya bisa dilakukan oleh artis',
        'Dilarang oleh pemerintah',
      ],
      correctAnswer: 0,
      explanation: 'Pemasaran mulut ke mulut (word of mouth) sangat efektif karena gratis dan rekomendasi teman biasanya lebih dipercaya.',
    },
    {
      id: 'pkwu-q43',
      question: 'Manakah yang termasuk bahan finishing untuk kerajinan kayu atau batok kelapa?',
      options: [
        'Tepung terigu',
        'Pernis atau pelitur',
        'Air gula',
        'Minyak goreng',
      ],
      correctAnswer: 1,
      explanation: 'Pernis atau pelitur digunakan untuk melapisi kerajinan kayu agar mengkilap, tahan lama, dan seratnya lebih menonjol.',
    },
    {
      id: 'pkwu-q44',
      question: 'Jika ingin membuat kerajinan "topeng" dari kertas bekas, teknik yang paling tepat adalah...',
      options: [
        'Bubur kertas (Paper Mache)',
        'Origami',
        'Anyaman',
        'Jahit tindas',
      ],
      correctAnswer: 0,
      explanation: 'Bubur kertas (kertas direndam air lalu dihancurkan dan campur lem) sangat mudah dibentuk (molding) menjadi bentuk 3D seperti topeng.',
    },
    {
      id: 'pkwu-q45',
      question: 'Apa yang dimaksud dengan produk "Hias"?',
      options: [
        'Produk yang hanya mengutamakan keindahan visual/estetika',
        'Produk yang bisa dipakai kerja',
        'Produk masakan',
        'Produk mesin',
      ],
      correctAnswer: 0,
      explanation: 'Produk hias (dekoratif) dibuat dengan tujuan utama sebagai pajangan atau penghias ruangan, bukan untuk fungsi pakai.',
    },
    {
      id: 'pkwu-q46',
      question: 'Limbah organik basah seperti kulit buah lunak kurang cocok untuk kerajinan bangun datar karena...',
      options: [
        'Baunya harum',
        'Kadar air tinggi dan cepat membusuk',
        'Warnanya bagus',
        'Mudah didapat',
      ],
      correctAnswer: 1,
      explanation: 'Limbah basah (kadar air tinggi) sulit dijadikan kerajinan awet karena proses pembusukan yang cepat.',
    },
    {
      id: 'pkwu-q47',
      question: 'Dalam merancang produk, faktor "Ergonomis" berarti produk tersebut harus...',
      options: [
        'Berasal dari luar negeri',
        'Nyaman dan aman saat digunakan oleh manusia',
        'Sangat berat dan besar',
        'Berbahaya tajam',
      ],
      correctAnswer: 1,
      explanation: 'Ergonomis berkaitan dengan kenyamanan, keamanan, dan kesehatan pengguna saat memakai produk tersebut.',
    },
    {
      id: 'pkwu-q48',
      question: 'Contoh inovasi produk dari limbah kain perca sisa konveksi adalah...',
      options: [
        'Keset kaki, bedcover, dan bros',
        'Pakan ikan lele',
        'Bahan bangunan beton',
        'Keripik kain',
      ],
      correctAnswer: 0,
      explanation: 'Kain perca bisa disambung menjadi keset, selimut (bedcover), atau dibentuk bunga menjadi bros hijab.',
    },
    {
      id: 'pkwu-q49',
      question: 'Mengapa riset pasar penting sebelum memulai usaha kerajinan?',
      options: [
        'Supaya bisa meniru tetangga',
        'Untuk mengetahui selera dan kebutuhan konsumen saat ini',
        'Agar rugi besar',
        'Untuk membuang waktu',
      ],
      correctAnswer: 1,
      explanation: 'Riset pasar membantu kita memahami apa yang sedang tren dan apa yang dibutuhkan pembeli, sehingga produk kita lebih mudah terjual.',
    },
    {
      id: 'pkwu-q50',
      question: 'Sikap mental yang harus dimiliki wirausahawan kerajinan limbah adalah...',
      options: [
        'Mudah putus asa dan malu',
        'Kreatif, inovatif, dan pantang menyerah',
        'Suka mengeluh',
        'Menunggu bantuan pemerintah saja',
      ],
      correctAnswer: 1,
      explanation: 'Wirausahawan sukses harus kreatif mencari ide, inovatif mengembangkan produk, dan ulet/pantang menyerah menghadapi kendala.',
    },
  ],
  glossary: {
    'Limbah Bangun Datar': 'Bahan sisa atau sampah yang memiliki bentuk dasar dua dimensi, seperti kertas, kardus, plastik lembaran, kain perca.',
    'Upcycling': 'Proses mengubah limbah atau produk yang tidak berguna menjadi bahan atau produk baru yang memiliki kualitas, kegunaan, atau nilai lingkungan yang lebih baik.',
    'Green Economy': 'Model ekonomi yang berfokus pada pembangunan berkelanjutan tanpa merusak lingkungan.',
    'Peluang Usaha': 'Kesempatan untuk memulai atau mengembangkan bisnis yang memiliki potensi untuk menghasilkan keuntungan.',
    'USP (Unique Selling Point)': 'Keunikan atau nilai tambah yang membedakan suatu produk dari produk pesaing.',
    'Analisis SWOT': 'Metode analisis yang mengidentifikasi Strengths (Kekuatan), Weaknesses (Kelemahan), Opportunities (Peluang), dan Threats (Ancaman).',
    'Target Pasar': 'Kelompok konsumen yang menjadi sasaran penjualan produk atau jasa.',
    'Margin Keuntungan': 'Selisih antara harga jual dengan biaya produksi.',
    'Ekonomi Sirkular': 'Sistem ekonomi yang meminimalkan limbah dengan menggunakan kembali, memperbaiki, dan mendaur ulang produk.',
    'Storytelling': 'Teknik pemasaran dengan menceritakan kisah di balik produk untuk membangun koneksi emosional.',
    'Eco-Branding': 'Strategi branding yang menekankan nilai-nilai ramah lingkungan.',
    'Kain Perca': 'Sisa potongan kain dari proses pembuatan pakaian atau kerajinan tekstil.',
    'Plastik Sachet': 'Kemasan plastik kecil berbentuk lembaran yang biasa digunakan untuk produk sekali pakai.',
    'Quality Control': 'Proses pemeriksaan kualitas produk untuk memastikan standar yang ditetapkan.',
    'Project Based Learning (PBL)': 'Model pembelajaran yang berpusat pada proyek sebagai media untuk mencapai tujuan pembelajaran.',
  },
};

export const sumberDayaModule: Module = {
  id: 'pkwu-sumber-daya',
  title: 'SUMBER DAYA USAHA, ADMINISTRASI DAN PEMASARAN',
  subtitle: 'KERAJINAN USAHA DARI BAHAN LIMBAH',
  description: 'Mempelajari Sumber Daya Usaha, Administrasi dan Pemasaran Kerajinan Usaha dari Bahan Limbah.',
  level: 'XI',
  phase: 'F',
  lessonNumber: 2,
  model: 'Project Based Learning (PBL)',
  activity: 'Perencanaan usaha kerajinan berbasis limbah',
  product: 'Proposal perencanaan usaha kerajinan dari bahan limbah',
  meaningfulUnderstanding: 'Memahami bahwa perencanaan usaha yang matang—mulai dari analisis 6M, administrasi yang rapi, hingga strategi pemasaran yang jitu—adalah fondasi utama untuk mengubah limbah menjadi produk bernilai ekonomi tinggi yang berkelanjutan.',
  objectives: [
    'Menjelaskan sumber daya usaha kerajinan dari bahan limbah.',
    'Mengidentifikasi sumber daya usaha kerajinan dari bahan limbah dilingkungan sekitar tempat tinggal.',
    'Mengidentifikasi administrasi yang harus dipersiapkan dalam usaha kerajinan dari bahan limbah.',
    'Menentukan strategi pemasaran yang cocok untuk kegiatan usaha kerajinan dari bahan limbah.',
  ],
  prerequisites: [
    '3.1.3 Mengidentifikasi sumber daya yang dibutuhkan dalam usaha kerajinan dari bahan limbah berbentuk bangun datar',
    '3.1.4 Menentukan administrasi dan pemasaran usaha kerajinan dari bahan limbah berbentuk bangun datar',
    '3.1.5 Menentukan komponen dan langkah-langkah dalam penyusunan perencanaan usaha kerajinan dari bahan limbah berbentuk bangun datar.',
  ],
  duration: '4 JP (2 Pertemuan)',
  sections: [
    {
      id: 'sumber-daya-usaha',
      title: 'Sumber Daya Usaha (6M)',
      icon: '💪',
      content: `
# SUMBER DAYA USAHA

Apabila kalian sudah menentukan produk kerajinan berdasarkan analisis peluang usaha, kalian dapat menentukan sumber daya yang dibutuhkan dalam menjalankan usaha tersebut. Dalam perencanaan proses produksi diperlukan pengelolaan yang baik untuk mencapai tujuan perusahaan. Sumber daya yang dimiliki oleh perusahaan dapat dikategorikan atas enam tipe sumber daya (6M), sebagai berikut:

### 1. Man (Manusia) 🧑‍🤝‍🧑
Dalam pendekatan ekonomi, sumber daya manusia adalah salah satu faktor produksi selain tanah, modal, dan keterampilan. Pandangan yang menyamakan manusia dengan faktor-faktor produksi lainnya dianggap tidak tepat, baik dilihat dari konsepsi, filsafat, maupun moral. Manusia merupakan unsur manajemen yang penting dalam mencapai tujuan perusahaan.

### 2. Money (Uang) 💰
Money atau uang merupakan salah satu unsur yang tidak dapat diabaikan. Besar kecilnya hasil kegiatan dapat diukur dari jumlah uang yang beredar dalam perusahaan. Oleh karena itu, uang merupakan unsur yang penting untuk mencapai tujuan perusahaan, karena segala sesuatu harus diperhitungkan secara rasional. Hal ini akan berhubungan dengan berapa uang yang harus disediakan untuk membiayai gaji tenaga kerja, alat-alat yang dibutuhkan dan harus dibeli, serta berapa hasil yang akan dicapai oleh perusahaan.

### 3. Material (Fisik) 🧱
Perusahaan umumnya tidak menghasilkan sendiri bahan mentah yang dibutuhkan, tetapi membeli dari pihak lain. Untuk itu manajer perusahaan berusaha untuk memperoleh bahan mentah dengan harga paling murah dan bahan bagus, dengan menggunakan cara pengangkutan yang murah dan membuat proses pengolahan menjadi seefisien mungkin.

### 4. Machine (Teknologi) ⚙️
Mesin memiliki peranan penting dalam proses produksi. Setelah revolusi industri, banyak pekerjaan manusia yang digantikan oleh mesin. Perkembangan teknologi yang begitu pesat menyebabkan penggunaan mesin semakin meningkat.

### 5. Method (Metode) 📋
Metode sangat dibutuhkan agar mekanisme kerja berjalan efektif dan efisien. Metode kerja yang sesuai dengan kebutuhan perusahaan, baik yang menyangkut proses produksi maupun administrasi tidak terjadi begitu saja melainkan memerlukan waktu yang lama.

### 6. Market (Pasar) 🛒
Jika barang yang diproduksi tidak laku, proses produksi barang akan berhenti. Penguasaan pasar dalam menyebarkan hasil produksi merupakan faktor penentuan dalam perusahaan. Pasar dapat dikuasai dengan cara kualitas barang harus sesuai dengan selera konsumen dan harga terjangkau oleh daya beli konsumen.
`
    },
    {
      id: 'administrasi-usaha-1',
      title: 'Pengertian & Tujuan Administrasi',
      icon: '📝',
      content: `
# ADMINISTRASI DAN PEMASARAN USAHA KERAJINAN DARI BAHAN LIMBAH

## 1. Persiapan Administrasi Usaha Sebelum Membuat Kerajinan dari Limbah

Administrasi adalah usaha atau kegiatan yang berkenaan dengan penyelenggaraan kebijaksanaan untuk mencapai tujuan. Administrasi dalam arti sempit adalah kegiatan yang meliputi: catat-mencatat, surat-menyurat, pembukuan ringan, ketik-mengetik, agenda, dan sebagainya yang bersifat teknis ketatausahaan.

Administrasi dalam arti luas adalah seluruh proses kerja sama antara dua orang atau lebih dalam mencapai tujuan dengan memanfaatkan sarana dan prasarana tertentu secara berdaya guna dan berhasil guna Kegiatan administrasi banyak mengandalkan catatan yang dapat berupa formulir, surat – surat, dan laporan.

a. **Formulir** banyak digunakan untuk mengurus atau menyelesaikan surat – surat tertentu.
b. **Surat** merupakan alat komunikasi tertulis yang berasal dari satu pihak dan ditujukan kepada pihak lain untuk menyampaikan pesan.
c. **Laporan** merupakan bentuk catatan kantor yang memperkuat hubungan vertical secara structural dari bawahan ke pimpinan.

## 2. Persiapan Administrasi Usaha Sebelum Membuat Perusahaan atau Memulai Berbisnis

### a. Persiapan Administrasi
Administrasi bisa dikatakan sebagai membatu, melayani, memenuhi, dan melengkapi. Kegiatan administrasi banyak mengandalkan catatan yang dapat berupa formulir, surat – surat, dan laporan. Beberapa Fungsi Administrasi:
1) Tujuan yang ingin diperoleh disetarakan dengan beban kerja yang ada.
2) Pekerjaan terbagi secara efektif.
3) Fungsi administrasi sebagai tata usaha adalah mempermudah pekerjaan laporan atau pengarsipan dokumen sehingga mempermudah kerja manaterial.
4) Perusahaan lebih terarah dan lebih terstruktur bila bekerjasama dalam kelompok kerja.
5) Manajemen yang tepat akan menyelesaikan tugas lebih efisien.

### b. Maksud Dan Tujuan Administrasi
Administrasi yang baik dan rapi adalah membantu pengelolaan perusahaan dan kelancaran usaha, khususnya dalam pelaporan hasil usaha dan pencatatan administrasi.

**Tujuan diterapkannya administrasi yaitu:**
1) Mendapatkan informasi proses bisnis perusahaan.
2) Akan mendapatkan data yang akurat dalam mengambil keputusan strategic decision making process seperti keputusan penetapan harga, keputusan efisien, keputusan investasi, dan keputusan permodalan.
3) Penyusunan program dalam rencana pengembangan usaha seperti lisensi atau waralaba franchise
4) Mengetahui kinerja perusahaan.
5) Memperlancar proses antar bagi dalam menjanlakan pekerjaan.

**Kegunaan utama dari catatan administrasi perusahaan yaitu:**
1) Administrasi di gunakan sebagai alat bukti catatan
2) Administrasi di gunakan sebagai alat manajemen laporan
3) Administrasi di butuhkan sebagai penilaian catatan dan laporan
`
    },
    {
      id: 'administrasi-usaha-2',
      title: 'Kegiatan & Aspek Administrasi',
      icon: '📂',
      content: `
### c. Kegiatan Administrasi

Kegiatan administrasi atau tata usaha meliputi pekerjaan pencatatan yang perlu dilakukan dalam perusahaan seperti:
1) Mencatat penyelenggaraan produksi
2) Menyusun rencana anggaran perusahaan
3) Melakukan surat-menyurat kedalam dan keluar perusahaan
4) Membuat daftar gaji karyawan
5) Mencatatan pesanan
6) Menyelenggarakan pembukuan
7) Melakukan pengarsipan dokumen

<br>

### Beberapa aspek penting dalam perencanaan administrasi usaha:

**a. Mengurus Izin Usaha**
Izin usaha adalah alat untuk membina, mengarahkan, mengawasi dan melindungi pengelolaan usaha.

**b. Penetapan Besarnya Retribusi**
Ketentuan tata cara perhitungan retribusi SITU

**c. Surat Izin Usaha Perdagangan (SIUP)**

**d. Pengurusan Pajak**

**e. Analisis Mengenai Dampak Lingkungan (AMDAL)**
`
    },
    {
      id: 'perencanaan-pemasaran',
      title: 'Perencanaan Pemasaran',
      icon: '📈',
      content: `
# d. Perencanaan Pemasaran Usaha Kerajinan dari Bahan Limbah

Langkah awal sebelum memulai sebuah usaha kerajinan dari bahan limbah adalah menentukan segmentasi pasar. Ada dua faktor utama yang bisa dimanfaatkan untuk menentukan segmentasi pasar dalam merencanakan pemasaran kerajinan dari bahan limbah yaitu:

### 1) Lokasi Usaha 📍
Segmentasi pasar bisa dibedakan berdasarkan lokasi usaha tersebut berada. Oleh sebab itu, perhatikan kebutuhan dan minat konsumen di sekitar lokasi usaha agar produk yang ditawarkan sesuai dengan permintaan para konsumen.

### 2) Harga jual 🏷️
Harga jual produk juga menjadi salah satu faktor utama dalam membedakan segmentasi pasar. Para pelaku usaha biasanya membagi target pasar menjadi tiga kelompok, yaitu kelompok konsumen bawah, konsumen menengah, dan konsumen segmen atas. Strategi pemasaran untuk setiap target pasar tentu saja berbeda.

<br>

> Dengan menentukan segmentasi pasar sebelum memulai usaha, maka secara tidak langsung telah memilih fokus usaha yang ingin dioptimalkan untuk meningkatkan daya saing dan strategi pemasaran yang paling efektif untuk memulai usaha makanan internasional. Penguasaan pasar dalam arti menyebarkan hasil produksi merupakan faktor menentukan dalam perusahaan. Agar pasar dapat dikuasai maka kualitas dan harga makanan harus sesuai dengan selera konsumen dan daya beli (kemampuan) konsumen. Faktor pemasaran dapat dikatakan berhasil jika jangkauan pasar semakin luas dan masa produksi dapat bertahan dalam waktu yang lama. Untuk itu hal-hal yang perlu dipertimbangkan, meliputi sasaran pasar, selera konsumen, citra produk, saluran distribusi, dan penentuan harga.
`
    },
    {
      id: 'kegiatan-pemasaran',
      title: '5 Kegiatan Pemasaran',
      icon: '📢',
      content: `
# Rencana Pemasaran

Ada lima kegiatan dan rencana pemasaran yang bisa dilakukan untuk mengomunikasikan produk dan merk usaha, yaitu sebagai berikut:

### a. Penjualan personal (personal selling) 🤝
Merupakan bentuk komunikasi yang menggunakan media individu.

### b. Iklan (advertising) 📺
Merupakan komunikasi produk melalui media dan tidak dilakukan secara individu atau perorangan.

### c. Promosi penjualan (sales promotion) 🎁
Dapat digunakan untuk memasarkan dan mengomunikasikan pesan produk anda kepada calon konsumen.
Promosi meliputi kegiatan perencanaan, implementasi dan pengendalian komunikasi dari suatu organisasi kepada konsumen serta sasaran lainya.

### d. Publikasi (publication) 📰
Mencakup pengaturan komunikasi masa diluar iklan dan promosi penjualan yang bertujuan untuk meningkatkan penjualan atau memperkuat merek secara tidak langsung dan tidak bersifat menjual.

### e. Sponsorship 🏆
Merupakan aplikasi dalam mempromosikan produk atau merek yang berasosiasi dengan kegiatan perusahaan lain atau kegiatan pemerintah dan masyarakat.
`
    }
  ],
  videos: [
    {
      id: 'video-sumber-daya',
      title: 'Sumber Daya Usaha',
      url: 'https://www.youtube.com/embed/sdgyMRNqB7g?si=ADbDbyS8dLK0U52Y',
      description: 'Video pembelajaran tentang Sumber Daya Usaha, Administrasi, dan Pemasaran Kerajinan.',
    }
  ],
  quizQuestions: [],
  glossary: {},
  reflectionQuestions: [
    {
      id: 1,
      question: 'Bagian materi mana yang paling kamu pahami tentang usaha kerajinan dari bahan limbah? Mengapa bagian tersebut menurutmu penting?',
      hint: 'Identifikasi konsep yang paling mudah kamu mengerti.'
    },
    {
      id: 2,
      question: 'Menurutmu, apa manfaat mencatat kegiatan dan keuangan usaha dalam menjalankan usaha kerajinan dari bahan limbah?',
      hint: 'Pikirkan tentang keuntungan memiliki catatan yang rapi.'
    },
    {
      id: 3,
      question: 'Mengapa perencanaan usaha yang baik diperlukan agar usaha kerajinan dari bahan limbah dapat berjalan terus dan tidak berhenti di tengah jalan?',
      hint: 'Hubungkan dengan keberlanjutan bisnis.'
    },
    {
      id: 4,
      question: 'Mengapa produk kerajinan perlu disesuaikan dengan kebutuhan, selera, dan kemampuan beli konsumen? Jelaskan dengan pendapatmu sendiri.',
      hint: 'Kaitkan dengan kepuasan pelanggan dan penjualan.'
    },
    {
      id: 5,
      question: 'Setelah mempelajari materi ini, sikap atau keterampilan apa yang ingin kamu tingkatkan jika suatu saat ingin membuka usaha kerajinan dari bahan limbah?',
      hint: 'Refleksi diri tentang potensi wirausahamu.'
    }
  ]
};


export const rencanaUsahaModule: Module = {
  id: 'rencana-usaha',
  title: 'RENCANA USAHA KERAJINAN DARI BAHAN LIMBAH',
  subtitle: 'Perencanaan dan Analisis Usaha',
  description: 'Mempelajari cara menyusun rencana usaha yang matang untuk kerajinan dari bahan limbah, termasuk analisis peluang, strategi pemasaran, dan perhitungan keuangan.',
  level: 'XI',
  phase: 'F',
  lessonNumber: 3,
  model: 'Project Based Learning (PBL)',
  activity: 'Menyusun Proposal Usaha',
  product: 'Dokumen Perencanaan Usaha',
  meaningfulUnderstanding: 'Perencanaan usaha yang matang adalah peta jalan menuju kesuksesan. Hal ini membantu mengubah ide kreatif menjadi bisnis yang berkelanjutan dan meminimalkan risiko kegagalan.',
  objectives: [
    'Menentukan komponen perencanaan usaha kerajinan dari bahan limbah',
    'Menentukan langkah-langkah penyusunan proposal perencanaan usaha kerajinan dari bahan limbah',
    'Membuat proposal perencanaan usaha kerajinan dari bahan limbah',
  ],
  prerequisites: [
    'Memahami jenis dan karakteristik bahan limbah',
    'Memahami analisis peluang usaha (SWOT)',
    'Memahami konsep dasar sumber daya usaha (6M)',
  ],
  duration: '4 JP (2 Pertemuan)',
  sections: [
    {
      id: 'komponen-perencanaan-1',
      title: 'Komponen Perencanaan Usaha (Bagian 1)',
      icon: '📝',
      content: `
# Komponen Perencanaan Usaha

Dalam menyusun rencana bisnis terdapat 7 komponen perencanaan usaha. Semua bagian tersebut penting, saling terkait dan berhubungan dalam kegiatan perencanaan bisnis sehingga menghasilkan sebuah persiapan yang baik untuk memulai usaha.

Berikut adalah komponen dalam menyusun sebuah rencana:

## 1. Ulasan Deskripsi Bisnis
Di sini kalian harus menjelaskan secara singkat bidang usaha apa yang akan dijalankan. Tuliskan potensi produk kalian saat ini dan kemungkinannya dimasa depan. Selain itu, berikan informasi peluang pasarnya serta perkembangan produk untuk bisa bertahan dan menyesuaikan dengan pasar yang ada.

## 2. Strategi Pemasaran
Strategi pemasaran yang dijalankan harus sesuai dengan hasil analisis pasar yang telah dilakukan dengan cermat. Analisis pasar adalah kekuatan yang harus kalian gunakan untuk menciptakan target pembeli. Kalian harus memahami seluruh aspek yang berkaitan dengan pasar sehingga target penjualan dapat ditentukan (ke mana produk kalian akan dipasarkan).

## 3. Analisis Pesaing
Analisis pesaing digunakan untuk mengetahui kekuatan dan kelemahan pesaing kalian dalam satu pasar yang sama. Setelah menemukan kekuatannya, kemudian mencari strategi untuk memasarkan produk dengan cara yang berbeda dengan pesaing. Kalian juga harus mencari strategi untuk menghalangi pesaing masuk dan meniru strategi yang sama dengan kalian. Demikian juga dengan kelemahan yang ditemukan, dapat dieksploitasi dengan mengembangkan produk yang lebih baik dari pesaing kalian.

## 4. Rencana Desain dan Pengembangan
Rencana desain dan pengembangan diperlukan untuk menunjukkan tahap perencanaan produk serta grafik pengembangan dalam konteks produksi dan penjualan. Ini berguna untuk membuat rencana anggaran biaya produksi yang sesuai dengan kebutuhan.
      `,
    },
    {
      id: 'komponen-perencanaan-2',
      title: 'Komponen Perencanaan Usaha (Bagian 2)',
      icon: '📊',
      content: `
# Komponen Perencanaan Usaha (Lanjutan)

## 5. Rencana Operasional dan Manajemen
Rencana operasional dan manajemen dibuat untuk menjelaskan bagaimana usaha akan berjalan dan berkelanjutan. Rencana operasional akan berfokus pada kebutuhan logistik perusahaan. Seperti tugas dan tanggung jawab tim manajemen, bagaimana prosedur usahanya, serta kebutuhan anggaran.

## 6. Pembiayaan
Faktor pembiayaan menjadi unsur paling penting dalam sebuah rencana usaha. Hal ini diperlukan untuk mengetahui dari mana sumber dana berasal dan cara mengatur anggaran usaha.

## 7. Kesimpulan Usaha
Kalian dapat menampilkan jadwal waktu tiap komponen yang akan dilakukan. Perkiraan waktu dan hal-hal penting lainnya yang akan mendukung segala aktivitas dalam memulai usaha.
      `,
    },
    {
      id: 'langkah-penyusunan-1',
      title: 'Langkah Penyusunan Proposal (Bagian 1)',
      icon: '🛠️',
      content: `
# Langkah-langkah Penyusunan Proposal

Di dalam perencanaan suatu usaha terdapat langkah-langkah dalam penyusunan proposal/perencanaan usaha kerajinan dari bahan limbah yang baik, antara lain:

## 1. Pemilihan Jenis Usaha
Jenis usaha yang dipilih yaitu yang telah melalui hasil analisis dengan memperhatikan banyak faktor.

## 2. Nama Perusahaan
Pemilihan nama perusahaan yaitu nama yang mudah dikenal atau menunjukkan makna tertentu.

## 3. Lokasi Usaha
Lokasi usaha yang dipilih merupakan lokasi yang dekat dengan bahan baku, tidak jauh dari lokasi rumah pengelola, dan tidak terlalu jauh dari jangkauan pasar yang akan dituju.

## 4. Perizinan Usaha
Izin usaha yang disiapkan antara lain:
- NPWP dari kantor pajak
- Akta notaris dari kantor notaris
- SIUP/TDP dari dinas perindustrian kota/kabupaten
- Izin PIRT dari dinas Kesehatan kota/kabupaten
      `,
    },
    {
      id: 'langkah-penyusunan-2',
      title: 'Langkah Penyusunan Proposal (Bagian 2)',
      icon: '👥',
      content: `
# Langkah-langkah Penyusunan Proposal (Lanjutan)

## 5. Sumber Daya Manusia (SDM)
SDM yang disiapkan terdiri atas tiga orang pendiri, yang mempunyai tugas masing-masing yaitu:
- Penanggung jawab produksi
- Penanggung jawab pemasaran
- Penanggung jawab administrasi dan keuangan

## 6. Aspek Produksi
Hal-hal yang harus diperhatikan pada aspek produksi adalah:
- Bahan baku dan bahan tambahan
- Peralatan yang digunakan
- Jumlah tenaga kerja
- Hasil produksi

## 7. Aspek Keuangan
Hal-hal yang harus diperhatikan pada aspek keuangan adalah:
- Biaya tidak tetap (Variable Cost)
- Biaya tetap (Fixed Cost)
- Total biaya
- Penerimaan kotor
- Pendapatan bersih (Laba)
      `,
    }
  ],
  videos: [
    {
      id: 'video-rencana-1',
      title: 'Perencanaan Usaha Kerajinan Limbah',
      url: 'https://www.youtube.com/embed/VIDEO_ID_HERE', // Ganti dengan ID video yang sesuai
      description: 'Simak video berikut untuk memahami lebih dalam tentang penyusunan rencana usaha kerajinan dari bahan limbah.',
      task: 'Kerjakan apa yang menjadi tugas di akhir video.',
    }
  ],
  quizQuestions: [],
  glossary: {},
  reflectionQuestions: []
};

export const pkwuModules: Module[] = [kerajinanLimbahModule, sumberDayaModule, rencanaUsahaModule];
